...
class StudentDashboard(LoginRequiredMixin,CreateView):
    ...
    def get(self,request,error=None):
        ...
        context = {'userform':self.userform,'prefform': self.prefForm,'pic':request.user.profilepic,'error':error}
        
        if Enroll.objects.filter(student=currentuserinfo).exists():
            enrolls = Enroll.objects.filter(student=currentuserinfo)
            context['enrolls'] = enrolls
        else:
            context['enrolls'] = None
        # get current user informations from database main code is in get_or_create.py    
        context['userform'] = get_user_information(currentuser,currentuserinfo)
    ...
    def post(self,request,error=None):
        user_info_form_data = self.userform(request.POST)
        if request.FILES:
